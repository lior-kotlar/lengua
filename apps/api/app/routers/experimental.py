...
every environment, so they are present in the code (and the OpenAPI contract) but **unreachable** in
prod until an operator flips the flag — and, because the flag resolves from the ``feature_flags``
table within ``FEATURE_FLAG_TTL_SECONDS``, **without a redeploy** (task 6.9.3).
