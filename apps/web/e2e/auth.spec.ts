...
test('a logged-out visit to / redirects to the login screen', async ({
  ...
  // Both OAuth buttons render; the committed default (VITE_OAUTH_PROVIDERS unset → ['google']) leaves
  // Google enabled and Apple DISABLED — Apple isn't configured in Supabase, so a real click would
  // dead-end on a raw 400 (finding S2). Re-enable per env via VITE_OAUTH_PROVIDERS=google,apple.
  await expect(
