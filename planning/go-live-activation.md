...
- **E1 — Set repo variable `DEPLOY_ENABLED=true`** (`gh variable set DEPLOY_ENABLED -b true`) — the committed CD jobs are gated off until this flips. **verify:** a trivial merge to `main` triggers the staging deploy run → green smoke probes (`/health`, `/ready`, web 200).
  - ⚠️ **BLOCKED (2026-06-29) — DB host is IPv6-only, GitHub runners are IPv4-only.** First armed run (push of PR #72) reached `migrate-staging` and failed with `OSError: [Errno 101] Network is unreachable` (asyncpg SSL connect). Cause: `SUPABASE_STAGING_DATABASE_URL` points at the Supabase **direct** host (`db.<ref>.supabase.co`, IPv6-only); GitHub-hosted runners have no IPv6. The deploy jobs SKIPPED (migrate gates them) so live staging was untouched. `DEPLOY_ENABLED` set back to `false` (disarmed) so `main` stays green until fixed.
  - **OWNER FIX (secret change):** switch `SUPABASE_STAGING_DATABASE_URL` **and** `SUPABASE_PROD_DATABASE_URL` to the Supabase **Session-mode pooler** (IPv4) string — Project Settings → Database → "Session pooler": `postgresql://postgres.<ref>:<pw>@aws-0-<region>.pooler.supabase.com:5432/postgres` (port **5432** = session mode, NOT 6543/transaction — alembic + the runtime SQLAlchemy/asyncpg engine both need session mode; it's a drop-in for the Cloud Run runtime, which uses the same secret). No code change — `app/db/migration_url.py` reads the var verbatim. Then re-arm (`DEPLOY_ENABLED=true`) and re-run the verify.
  - **Staging fixed 2026-06-29:** `SUPABASE_STAGING_DATABASE_URL` switched to the session pooler (`aws-0-eu-west-1.pooler.supabase.com:5432`); `DEPLOY_ENABLED` re-armed; this merge re-runs the staging CD verify. (`SUPABASE_PROD_DATABASE_URL` still needs the same change before §F prod promotion.)
