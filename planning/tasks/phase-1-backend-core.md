...
- [ ] **1.3.4** Implement `repositories/reviews.py` + `repositories/proficiency.py` (insert a review row, upsert proficiency) scoped by `user_id`.
...
- [ ] **1.3.5** Implement `repositories/settings.py` (per-user key/value) with read-all + upsert.
...
- [ ] **1.3.6** Build the service layer (`services/languages.py`, `services/generate.py`, `services/review.py`, `services/discover.py`, `services/proficiency.py`, `services/settings.py`) that orchestrates `lengua_core` + repositories and never emits raw SQL.
...
- [ ] **1.4.1** Configure Alembic (`apps/api/migrations/`, `env.py` async, target `Base.metadata`, `DATABASE_URL` from settings).
      verify: `alembic current` runs against a throwaway Postgres without error and reports an empty/initial state.
- [ ] **1.4.2** Author the first migration = full schema: `profiles`, `languages`, `cards` (+ index on `(user_id, language_id, saved, due)`), `reviews`, `proficiency`, `user_settings`, with UUID/timestamptz/jsonb types and the FKs/constraints from 1.3.2.
...
- [ ] **1.4.3** Add the `gemini_usage` (PK `user_id, day, kind`) and `gemini_budget` (PK `day`) tables to the migration — built now, used by the Phase 3 quota gate.
...
- [ ] **1.4.4** Add a seeded dev-user mechanism (a fixed dev UUID profile inserted via an idempotent seed script / data migration, used as `current_user` until Phase 2).
