...
- [x] **6.1.3** Create an Artifact Registry repo (EU region) and push the built image with both a git-SHA tag and `latest`. <!-- staging-validated 2026-07-05 (read-only): AR repo europe-west1-docker.pkg.dev/lengua-prod/lengua holds SHA-tagged images + a moving `:staging` tag (the staging pipeline's stand-in for `latest`). See CHANGELOG. -->
...
- [x] **6.1.4** Provision the `lengua-api-staging` Cloud Run service (EU region, scale-to-zero, min-instances 0, concurrency + CPU/memory set) and deploy the image.
...
- [x] **6.2.2** Apply the full Alembic migration history to the staging DB and confirm it is at head.
...
- [x] **6.6.1** Add the `main`-push workflow step that builds the API image and pushes it to Artifact Registry tagged with the commit SHA. <!-- as-code done: `.github/workflows/deploy-staging.yml` job `build-push` builds apps/api/Dockerfile and pushes `…/lengua/lengua-api:<sha>` + `:staging` to Artifact Registry; gated on DEPLOY_ENABLED (skips → main stays green). Staging-validated 2026-07-05 (read-only): `build-push` green on the latest main deploy (run 28715034653); AR holds the `<sha>` + `:staging` tags. See CHANGELOG. -->
...
- [x] **6.6.2** Add the step that deploys the freshly pushed image to `lengua-api-staging` as a new revision. <!-- as-code done: job `deploy-api-staging` (composite `.github/actions/cloud-run-deploy`) deploys the SHA image with staging runtime env (LLM_PROVIDER=groq + staging DB/Supabase/OTEL/Sentry), scale-to-zero, max-instances=1 (in-process cost guard), startup+liveness probes → /health; gated on DEPLOY_ENABLED. Staging-validated 2026-07-05 (read-only): `deploy-api-staging` green on the latest main deploy; newest revision serving 100%, staging /health 200. See CHANGELOG. -->
...
- [x] **6.6.3** Add a **discrete, logged** Alembic migration step that runs against the staging DB before/with the deploy (job-level, not in the request path). <!-- CI-VERIFIABLE MECHANISM DONE + TICKED (per the precedent of 6.1.2): the `alembic -x env=staging|prod|local` resolver (`app/db/migration_url.py`, mapping STAGING_DATABASE_URL / PROD_DATABASE_URL / DATABASE_URL; `-x db_url=` still wins) + its precedence unit test (`tests/db/test_migration_url.py`, 100% line+branch) are green in CI, and the discrete `migrate-staging` job (a distinct, logged `alembic -x env=staging upgrade head` job) is committed in `.github/workflows/deploy-staging.yml`, gated on DEPLOY_ENABLED. The LIVE half — that job running `upgrade head` against hosted staging on a real merge so `alembic -x env=staging current` == `heads` — is owner-deferred §12. -->
      ...
      depends: 6.2.2
- [x] **6.6.4** Add the step that deploys `apps/web` to Vercel **staging** on merge to `main`. <!-- as-code done: job `deploy-web-staging` (vercel pull/build/deploy --prebuilt) ships only client-safe VITE_* (anon key + public URLs + the staging Cloud Run URL), gated on DEPLOY_ENABLED. Staging-validated 2026-07-05 (read-only): `deploy-web-staging` green on the latest main deploy; `lengua-staging.vercel.app` serves 200 (stable alias, S18). The owner-account Vercel *link* itself (6.3.1) stays owner-tracked. See CHANGELOG. -->
...
- [x] **6.6.5** Smoke-check the staging deploy at the end of the workflow (probe `/health` + `/ready` + a web `200`) and fail the run if any probe fails. <!-- as-code done: job `smoke-staging` (composite `.github/actions/cloud-run-smoke`) probes /health + /ready + web 200 and fails the run on any non-2xx; gated on DEPLOY_ENABLED. Staging-validated 2026-07-05 (read-only): `smoke-staging` green on the latest main deploy (run 28715034653). See CHANGELOG. -->
...
- [x] **6.8.1** Configure Cloud Run revision retention + traffic tags so the previous good revision is always kept and addressable. <!-- as-code done: Cloud Run retains prior revisions by default (the deploy jobs never delete them), so the previous good revision is always addressable by name for rollback; `infra/deploy/README.md` + the runbook document optionally tagging a revision (`--set-tags prev=<rev>`) for a stable preview URL. Staging-validated 2026-07-05 (read-only): `gcloud run revisions list` shows 11+ retained Ready revisions on lengua-api-staging (previous reachable by name). The prod-traffic rollback *proof* (6.8.2) needs a mutation and stays owner-tracked. See CHANGELOG. -->
...
- [x] **6.9.1** Add a feature-flag mechanism (env-driven flags and/or a small `feature_flags` table) with a typed accessor in the API and an exposure to the web client. <!-- DONE: `app/feature_flags.py` resolves each KNOWN flag from an env default (`FEATURE_*`, off by default) overlaid by a row in the new GLOBAL `feature_flags` table, cached in-process for `FEATURE_FLAG_TTL_SECONDS` (~30s) behind an INJECTABLE clock + a `get_feature_flags()` singleton dependency + `reset_feature_flags()` (mirrors app/ratelimit.py / app/discover_cache.py). Public `GET /feature-flags` returns the resolved PUBLIC map (allow-listed, secret-free); the web hook `apps/web/src/lib/feature-flags.ts` reads it. MIGRATION: Alembic 0005 + the canonical `supabase/migrations/20260628000000_feature_flags.sql` (round-trips on bare Postgres via `to_regrole` guards; covered by the schema round-trip + a new lockstep privilege test). SECURITY: `feature_flags` is GLOBAL config — REVOKE ALL from `authenticated`/`anon` + deny-by-default RLS (like `llm_budget`); writes are admin/service-role only; tests prove `authenticated` can neither write nor read it. -->
...
- [x] **6.9.2** Wrap at least one risky/new feature behind a flag defaulting **off in prod** so it ships dark. <!-- DONE: the experimental "word of the day" surface ships dark behind `word_of_the_day` (off by default everywhere). API half: `GET /experimental/word-of-the-day` is JWT-protected + flag-gated → 404 (absent) when off, 200 when on (`require_flag`). UI half: `<WordOfTheDay>` on the Dashboard renders nothing until the flag is on. Tests assert both off (absent) and on (present) on both halves. -->
      ...
      depends: 6.9.1
- [x] **6.9.3** Prove a flag can be toggled in prod **without a redeploy** (DB row / runtime config change reflected within one TTL/refresh). <!-- AS-CODE DONE: the mechanism + a deterministic TTL-refresh test ARE the no-redeploy proof — `test_table_snapshot_is_cached_within_the_ttl_then_refreshes` writes the `feature_flags` row, shows it is invisible within the TTL, then (after advancing the INJECTED clock past `FEATURE_FLAG_TTL_SECONDS`) the accessor reports the new value with no process restart. The LIVE "in prod, no Cloud Run revision change" confirmation needs a deployed service → owner-deferred (outstanding-work §12). -->
...
- [x] No secret leaks to the client and security scans pass — verify: the built web bundle contains only the anon key + public URLs (no service-role/LLM key), and `gitleaks` + `pip-audit` + `pnpm audit` pass on the gate (6.3.4 / 6.4.2 / 6.5.7).
