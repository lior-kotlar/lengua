...
- [x] **5.6.1** Build the **Service health** dashboard: request rate, error rate, and p50/p95/p99 latency per route. _(Committed as code: `infra/grafana/dashboards/service-health.json` — per-route request rate + 5xx rate from `http_server_duration_milliseconds_count`, overall p50/p95/p99 + per-route p95 from `..._bucket` via `histogram_quantile`, plus a 5xx-ratio stat. Portable: panels use a `${datasource}` template variable, not a pinned UID. **CI half DONE + ticked** — `apps/api/tests/obs/test_dashboards.py` proves the JSON is valid Grafana dashboard JSON AND every referenced metric maps to an emitted instrument (cross-ref discovered via AST of `app/llm_observability.py`+`app/product_metrics.py` + the FastAPI RED histogram). The **live "renders non-empty after a load script"** half needs the Phase-6 staging deploy + Grafana/Mimir creds (owner) — logged in `planning/outstanding-work.md` §11.)_
      ...
      depends: 5.2.6
- [x] **5.6.2** Build the **Gemini cost-guard** dashboard: calls/day vs budget ceiling, `gemini_budget_remaining`, quota blocks by reason, and tokens/day. _(Committed as code: `infra/grafana/dashboards/cost-guard.json`, the most important "stay free" dashboard. **NAMING reconciled** to the real provider-agnostic emitted names (§11): `gemini_budget_remaining`→`llm_budget_remaining` (gauge stat + over-time), calls/day = `sum(increase(llm_calls_total{result="success"}[1d]))` vs the budget-remaining ceiling, `quota_blocks_total{reason}`→`llm_cap_hits_total{gate}` (blocks-today stat + rate-by-gate), tokens/day = `llm_tokens_total{kind,direction}`. No `gemini_*` name used. **CI half DONE + ticked** via the drift/cross-ref test. The **live** half (drive generate + a forced block, watch `llm_budget_remaining` fall + a non-zero blocks panel on the rendered dashboard) needs the Phase-6 staging deploy + Grafana creds (owner) — logged in §11.)_
      ...
      depends: 5.2.4
- [x] **5.6.3** Build the **Product** dashboard: daily reviews, cards created, signups, active users, and CEFR level distribution. _(Committed as code: `infra/grafana/dashboards/product.json` — reviews/day (`reviews_total`), cards/day (`cards_created_total`), signups/day (`signups_total`), active-users now (`max(active_users)`), plus over-time series. **CEFR distribution:** no per-band metric is emitted yet, so it is a documented placeholder **text panel** (not a broken query) describing the future `proficiency_cefr_band` observable gauge — noted as a panel pending a metric, per the task. **CI half DONE + ticked** via the cross-ref test. The **live** half (seeded signup→generate→review run shows non-zero panels on the rendered dashboard) needs the Phase-6 staging deploy + Grafana creds (owner) — logged in §11.)_
      ...
      depends: 5.2.5
- [ ] **5.6.4** Build the **Infra** dashboard: Cloud Run instance count / cold starts, DB latency, and an error-logs (Loki) feed panel. _(LEFT OPEN — depends ENTIRELY on the Phase-6 Cloud Run deploy (no Cloud Run / instance metrics exist yet). A **skeleton** is committed (`infra/grafana/dashboards/infra.json`: panels + `${datasource}`/`${loki_datasource}` placeholders, tagged `phase6-skeleton`) so it is ready to wire once staging is live; the metric expressions are placeholders (Cloud Run / GCP-managed-Prometheus names finalize in Phase 6) and are intentionally **excluded** from the cross-ref test. Not tickable as code — logged in `planning/outstanding-work.md` §11.)_
