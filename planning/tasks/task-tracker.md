...
| 1 | [phase-1-backend-core.md](phase-1-backend-core.md) | FastAPI + Postgres core loop, LLM seam, OpenAPI | L | 0 | 34 | **in progress** — 1.1 done (lengua_core ported to a pure core: scheduler/proficiency/cards/config; SQLite relocated to legacy_streamlit/store; legacy app still runnable). 1.2 done (LLM provider seam: Groq default + Gemini reserved behind one Protocol, `get_provider()` fail-fast, shared retry/backoff + request caps; flip via `LLM_PROVIDER`). 1.3 in progress — 1.3a done (async SQLAlchemy 2.0 engine/sessionmaker + `get_db` in `app/db/`, typed ORM models for all 8 tables matching the canonical schema, reusable async `db_session` test fixture); 1.3.3–1.3.6 (repositories + service layer) next. 1.4–1.7 not started. |
