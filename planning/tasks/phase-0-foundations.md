...
- [x] Both apps build and run empty — verify: `cd apps/api && uv run uvicorn app.main:app` serves `GET /health` → 200, and `cd apps/web && pnpm build` produces a bundle that serves the placeholder page. ✓ Proven green on `main` CI run 28173871582 — the **build** job builds the API image + web Vite bundle; the **e2e** job runs the API container serving `GET /health`.
- [x] The legacy Streamlit app still runs after the restructure — verify: launching the relocated Streamlit entrypoint serves its home page with no ImportError. ✓ Verified at 0.1.2 and unchanged since (the CI-gate work touches `.github/`, `apps/api/app/`, `apps/api/Dockerfile` only — never `apps/api/legacy_streamlit/`).
...
- [x] Shared test infra works — verify: `uv run pytest` uses the FakeLLM + throwaway Postgres with no outbound LLM/network calls, and the E2E seed produces the demo account. ✓ Proven green on `main` CI run 28173871582 — the **backend tests** job runs pytest against the throwaway Postgres with FakeLLM (no LLM/network), and the **e2e** job seeds the demo account and asserts the LLM-call counter `0→>0` with zero real Groq/Gemini calls.
- [ ] Branch protection + DoD are enforced — verify: a direct push to `main` is rejected, opening a PR pre-fills the DoD template, and `gh api .../branches/main/protection` returns without 404. **DoD template ✓** (0.6.1 — pre-fills every PR). **Branch protection DEFERRED to launch** (0.6.3, OWNER — see [../owner-deferred-tasks.md](../owner-deferred-tasks.md)); the API returns 404 until the owner enables it. Non-blocking.
- [ ] All required free-tier accounts/secrets are ready — verify: outstanding owner items (0.6.3, 0.6.4, 0.7.7–0.7.10) all pass their checks; `gh secret list` includes `GCP_REGION` and `SENTRY_ORG`; Ben has Vercel/Grafana/Sentry access. Accounts ✓ (incl. Vercel single-seat + Grafana/Sentry — 0.7.9/0.7.10). **Pending OWNER (non-blocking):** secrets `GCP_REGION`/`SENTRY_ORG` (0.7.7) + Resend SMTP (0.7.8); Dependabot deferred (0.6.4). See [../owner-deferred-tasks.md](../owner-deferred-tasks.md).
- [x] every task above merged via a green PR with the quality gate held (≥80% coverage, E2E). ✓ All implementation tasks landed via green PRs (#1–#13); the only remaining items are OWNER-only actions (0.6.3/0.6.4/0.7.7/0.7.8), not code PRs.

**Phase 0 status: implementation COMPLETE.** Every agent-buildable task is merged green and the per-PR quality gate is live and green on `main`. The two unchecked exit conditions above are **owner-gated** (enable branch protection at launch; add `GCP_REGION`/`SENTRY_ORG` secrets + confirm Resend SMTP) and **do not block starting Phase 1**.
