...
- [x] **4.8.1** Settings screen: edit per-user daily new-card limit, daily total limit, and discover count; save via the settings endpoint with validation against server bounds.
      ...
      _Reconciliation: `PUT /settings` is a generic key/value store with no per-value bound. `discover_count` is validated against a REAL server bound (`DiscoverRequest.count` min/max from the OpenAPI schema, via `schemaLimits`); the two daily review limits have no schema bound, so they validate against the legacy app's product bounds (new 1–100, total 1–500) plus a cross-field `new ≤ total` rule, documented in `lib/settings.ts`. The review due batch now reads these per-user keys and bounds itself by them (task **4.8b**), each falling back to the `lengua_core` config default when unset/invalid — so editing them has a live effect (the earlier "tracked backend gap" is closed)._
...
- [x] **4.10.1** Shared loading + empty + error state components (skeletons, empty illustrations, retryable error cards) wired into the Generate/Review/Discover queries.
      ...
      _Done: shared `LoadingState` (skeleton + sr-only label, over a `ui/skeleton` primitive), `EmptyState` (icon + title + action, neutral/`success` tones), and `ErrorState` (retryable card) in `components/{loading-state,empty-state,error-state}.tsx`; the active-language context now exposes `refetch`, so Generate/Review/Discover render the shared skeleton (loading), empty card ("add a language" / "all caught up"), and a retryable error (languages query) — and Review's due-batch loading/empty/error route through the same components. vitest per screen asserts `loading-skeleton` / `empty-state` / `error-state` + retry; dedicated component tests cover each shared piece 100/100._
- [x] **4.10.2** A shared, reusable 429 "daily limit reached" state component used by Generate and Discover (and any future LLM-bound call), distinct from generic errors and tied to the quota response shape.
      ...
      _Done (formalized — the shared `DailyLimitPanel` already existed from 4.5/4.7): `daily-limit-panel.test.tsx` now asserts it renders ONLY for the quota-429 codes (`daily_cap_reached`/`daily_limit_reached`) and is inert for a non-quota 429 (`rate_limited`) and other errors; new `daily-limit-panel.shared.test.ts` scans the product source to prove the panel UI is defined in exactly one file and that Generate + Discover both reach it via the single shared `LlmErrorState` → `DailyLimitPanel` path (no duplicated 429 UI). No refactor needed — there was no duplication._
- [x] **4.10.3** First-run analytics-consent prompt: a consent banner/modal where PostHog (or any analytics) initializes **only after** explicit opt-in; the choice is persisted and re-prompts never fire once decided.
      ...
      _Done: `lib/analytics.ts` is the consent chokepoint — `initAnalytics()` boots the SDK at most once and ONLY when consent is `granted` AND a key (`VITE_POSTHOG_KEY`) is configured (PostHog is wired fully in Phase 5/8; default initializer is a documented no-op, so dev/CI/E2E load nothing even on opt-in — a clean seam behind the env flag). `AnalyticsConsentProvider` + `AnalyticsConsentBanner` (mounted app-global in `main.tsx`) show the first-run banner only while undecided and persist the choice in localStorage so it never re-prompts. vitest asserts no init before consent + exactly one init on opt-in; `e2e/consent.spec.ts` asserts the banner shows on first load, no analytics request fires, and the decision survives a reload._
...
- [x] A signed-in browser user can run the full core loop end-to-end against the API — generate → save → review (reveal + grade) → discover — with no Streamlit involved. — verify: a Playwright E2E spec runs the complete loop on the ephemeral stack (LLM stubbed) and passes in CI. _(Done: [`e2e/full-loop.spec.ts`](../../apps/web/e2e/full-loop.spec.ts) walks login (seeded demo) → Generate (FakeLLM sentences) → save (2 sentences = 4 cards) → Review (counts header + reveal + grade "Again", keeping the demo deck due) → Discover preview, in ONE session through the REAL server seam — nothing browser-stubbed — on the ephemeral FakeLLM stack; runs in the CI `e2e` job under `E2E_STACK=1`, zero real LLM calls.)_
- [x] Auth is complete: sign up, verify, log in, password reset, Google + Apple buttons, with token refresh + 401-retry working. — verify: Playwright auth spec covers signup→verify-notice, login, and a forced-401→refresh→retry path; all green in CI. _(Done: [`e2e/auth.spec.ts`](../../apps/web/e2e/auth.spec.ts) covers signup→verify-notice, demo login→home, sign-out re-gate, and **Google + Apple** buttons present + enabled on the login screen. **Reconciliation (4.3 decision):** the forced-401→refresh→retry path is covered exhaustively in vitest (`src/lib/api-client.test.ts`) rather than Playwright — a reliable mid-session 401 against a healthy local API isn't reproducible in E2E (documented in the auth-spec header). Password reset + email-verification landing are covered in vitest (`ResetPassword`/`ForgotPassword`/`AuthCallback` tests).)_
...
- [x] RTL + diacritics render correctly and tap-a-word works on touch and click for an RTL language. — verify: Playwright RTL spec (Arabic/Hebrew) passes visual + interaction assertions including a mid-sentence RTL word tap. _(Done: [`e2e/rtl.spec.ts`](../../apps/web/e2e/rtl.spec.ts) on the seeded vowelized **Hebrew** deck — `dir="rtl"` on the Review content region, `document.fonts.check('16px "Noto Sans Hebrew"')` + nikkud present + a visual snapshot attached, the vowel-marks toggle strips/restores nikkud, and a mid-sentence word tap via BOTH touch (`.tap()`) and click that opens the correct word's explanation.)_
- [x] The 429 daily-limit and analytics-consent experiences are first-class. — verify: vitest + Playwright assert the dedicated quota-429 panel and the consent-gated analytics init (no analytics before opt-in). _(Done: the single shared `DailyLimitPanel` is asserted for a stubbed quota-429 in both [`e2e/generate.spec.ts`](../../apps/web/e2e/generate.spec.ts) and [`e2e/discover.spec.ts`](../../apps/web/e2e/discover.spec.ts), plus vitest `daily-limit-panel.test.tsx` (renders ONLY for the `daily_cap_reached`/`daily_limit_reached` shape, inert otherwise); [`e2e/consent.spec.ts`](../../apps/web/e2e/consent.spec.ts) asserts the first-run banner shows, **no analytics request fires before a decision**, and the choice persists across reload, and vitest (`analytics-consent-*` + `lib/analytics`) asserts no init before consent and exactly one init on opt-in.)_
- [x] The typed API client is generated from `packages/api-types` and stays in sync. — verify: the CI drift check (`pnpm gen:api` produces no diff) passes on the merge commit. _(Done: the `lint + format + types` CI job runs `pnpm --filter api-types generate` then `git diff --exit-code` over `src/schema.ts` + `src/constants.ts`; the chain app-routes → `openapi.json` (asserted by `test_openapi_stable.py`) → `schema.ts`/`constants.ts` is enforced end to end. Clean on this branch (no backend/OpenAPI delta) and re-checked on the merge commit.)_
- [x] every task above merged via a green PR with the quality gate held (≥80% coverage, E2E). _(Done: all 44 Phase-4 task boxes (4.1–4.11) merged via green PRs; web coverage held at 100% line / ~99.7% branch over product code throughout. This milestone-closing PR adds [`e2e/full-loop.spec.ts`](../../apps/web/e2e/full-loop.spec.ts) and ticks the exit gate under the same green gate.)_
