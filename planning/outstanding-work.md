...
| ◐ | `docs/runbook.md` — **Health checks** (Phase 5) + **Deploy / Rollback / Run-a-migration / Rotate-a-secret / Budget-alert / Restore-from-backup** (Phase 6, this PR) now filled with concrete commands. Remaining: **On-call** + **Store-release checklist** finalized at launch (Phase 9); live execution owner-deferred (§12). | docs/runbook.md | Phase 9 (On-call/store) |
...
| | Fix | Where |
|---|---|---|
| ☑ | ~~Exit-gate bullet lists "outstanding owner items (0.6.3, 0.6.4, **0.7.7–0.7.10**)" but 0.7.9/0.7.10 are already done — narrow the range to **0.7.7–0.7.8**~~ — DONE (narrowed to `0.7.7–0.7.8`). | planning/tasks/phase-0-foundations.md:174 |
...
| 🔒 | **5.2.6 live half — query per-route p95 in Grafana/Mimir.** CI half DONE + ticked: `FastAPIInstrumentor.instrument_app` is given the app `MeterProvider`, so each request emits the `http.server.duration` histogram per route (`tests/obs/test_red_metrics.py` asserts it via the in-memory reader). The pinned instrumentation labels the histogram `http.target` (path) under the old HTTP semconv (the span still carries `http.route`); either is "per-route". The live confirmation — a Grafana Explore query for the request-duration histogram returning a per-route p95 series after a load script — needs the **Phase-6 staging deploy + Mimir/Grafana creds (owner)**. | apps/api/app/observability.py; tests/obs/test_red_metrics.py | deferred — owner + phase6 (CI half ticked; live Mimir query later) | 2026-06-27 |
| ◐ | **Naming reconciliation (llm.* / llm_* canonical; no gemini-prefixed names) — APPLIED in 5.2.** Dashboards (5.6) + docs reference the real emitted names. The plan's `quota_blocks_total{reason}` IS the shipped `llm_cap_hits_total{gate}` (gate == reason) — single counter, no duplicate. The plan's missing token counter SHIPPED as `llm_tokens_total{kind,direction}` (5.2.4), NOT `gemini_tokens_total`. The plan's `gemini.{generate,discover,explain}` spans are the single provider-agnostic `llm.call` span (5.2.1), extended with `llm.input_size` + `llm.retry_count`. No `gemini`-prefixed name exists anywhere. | apps/api/app/llm_observability.py; app/quota.py; app/product_metrics.py; planning/tasks/phase-5-observability.md (5.2/5.6) | decided + applied (groups 5.1 + 5.2) | 2026-06-27 |
| 🔒 | **5.2.5 product metrics are process-local (Phase-6 distributed swap).** `active_users` (rolling in-process window) + `signups_total` (process-local first-seen dedup) are per-instance — same caveat as the in-process rate limiter / discover cache. Accurate on one Cloud Run instance; Phase 6 moves the durable signals to the DB / a shared store. `signups_total` is a coarse server-side proxy wired at the post-login `/me`; the precise activation funnel is the consent-gated PostHog funnel (5.9.2). | apps/api/app/product_metrics.py; app/services/me.py | as-code done; durable/distributed version deferred to phase6 | 2026-06-27 |
| 🔒 | **5.3.1 live half — the structured log line appears in Grafana Loki.** As-code DONE + CI-verified: a module-owned OTel `LoggerProvider` + `LoggingHandler` (root + access loggers) routes stdlib logging through OTel, with a `BatchLogRecordProcessor(OTLPLogExporter())` attached ONLY when `OTEL_EXPORTER_OTLP_LOGS_ENDPOINT`/`…_OTLP_ENDPOINT` is set (no-op/zero-egress otherwise; same `build_resource` so logs carry `service_name=lengua-api`); the stdout JSON access line (Cloud-Run→Loki primary path) is kept and is valid JSON. `tests/obs/test_logging.py` asserts the wiring (no-op vs OTLP), the export-through-handler path (in-memory exporter), and the valid-JSON access line. The live check — the line actually shows up in Grafana Loki filtered by `service_name="lengua-api"` — needs the **Phase-6 staging Cloud Run deploy + Grafana Cloud Loki/OTLP creds (owner)**. | apps/api/app/observability.py; .env.example; tests/obs/test_logging.py | deferred — owner + phase6 (do NOT tick the live half until staging is live) | 2026-06-27 |
| 🔒 | **5.3.3 — Grafana Tempo→Loki "trace to logs" correlation (ENTIRELY live).** No CI-verifiable portion: it needs Tempo + Loki populated by a deployed service and the Grafana Tempo→Loki trace-to-logs data-link configured, then opening a trace and jumping to its logs by `trace_id`. The as-code prerequisites are done — every log record carries `trace_id`/`span_id` (5.3.2) and both trace + log OTLP export are wired (5.1.5 + 5.3.1). **Left OPEN, not ticked.** Needs the **Phase-6 staging deploy + Grafana creds (owner)**; depends 5.1.5 + 5.3.2. | planning/tasks/phase-5-observability.md (5.3.3); apps/api/app/observability.py | deferred — owner + phase6 (entirely live) | 2026-06-27 |
| ◐ | **`opentelemetry.sdk._logs.LoggingHandler` deprecation (revisit).** OTel ≥1.43 flags the SDK `LoggingHandler` with a relocation `DeprecationWarning` ("use the handler from `opentelemetry-instrumentation-logging`"). KEPT for 5.3.1: it is the standard stdlib→OTLP log bridge, the OTel logs signal is still officially unstable (the SDK's own `InMemoryLogExporter` warns it "WILL be removed"), and the suggested package is not a current dependency. CI does not treat warnings as errors (no `filterwarnings=error`), so it is non-fatal. Revisit (swap to the relocated handler, or add `opentelemetry-instrumentation-logging`) when the logs signal stabilizes / during Phase-6 deploy hardening. | apps/api/app/observability.py | decided (keep; revisit at logs-signal stabilization) | 2026-06-27 |
| 🔒 | **5.5.1 / 5.5.2 live halves — browser client span as the trace root in Tempo (+ browser-span export).** As-code DONE + CI-verified: the web client injects a fresh, minimal W3C `traceparent` per API request (`apps/web/src/lib/trace.ts` `generateTraceparent` + `traceMiddleware` in `api-client.ts`, preserved across the 401 refresh/retry; vitest + a `generate.spec.ts` network assertion), and the backend CONTINUES it into ONE client→API→DB/LLM trace (`apps/api/tests/obs/test_trace_continuation.py`, `@integration`: server span trace-id == supplied, parented under the client span-id; SQLAlchemy + `llm.call` children share it). **RECONCILIATION:** chose minimal `traceparent` propagation over a heavy `@opentelemetry` browser SDK (bundle cost); under `FakeLLM` the provider signal is the `llm.call` span, not an httpx/`gemini.generate` span (the plan predates the LLM seam). Today the browser only STARTS the trace via the header — it does NOT yet EXPORT its own client span. The live verifies — in Grafana Tempo the trace begins with a **browser/client span as the root**, and the assembled client→API→DB/LLM trace is searchable — need (a) a web OTLP SDK exporting the browser span to a deployed collector and (b) the Phase-6 staging deploy + Grafana Cloud Tempo creds (owner). Add the browser-span-export-to-Tempo enhancement in Phase 6; the propagation/continuation it builds on is done now. | apps/web/src/lib/trace.ts; apps/web/src/lib/api-client.ts; apps/api/tests/obs/test_trace_continuation.py | deferred — owner + phase6 (CI halves ticked; browser-span export + Tempo assembly later) | 2026-06-27 |
| 🔒 | **5.4.1 live half — the Sentry issue's `trace_id` resolves to a Grafana Tempo trace.** CI half DONE + ticked: `app/error_tracking.py` inits Sentry only when `SENTRY_DSN_API` is set (no-op/zero-egress otherwise), binds the request `user.id` + a `trace_id` tag (active OTel span) onto the isolation scope, and `tests/obs/test_sentry.py` proves the deliberately-failing `GET /__test__/debug-error` is captured carrying `user_id` + a 32-hex `trace_id` via an in-memory transport. The live check — the exception shows up as a Sentry **issue** whose `trace_id` actually opens the matching Tempo trace — needs the **Phase-6 staging deploy + the live backend Sentry DSN (`SENTRY_DSN_API`) + Grafana Cloud Tempo creds (owner)**, plus the `GROQ`/secrets already noted. The `SENTRY_DSN_API` CI/deploy secret + the Sentry↔Tempo trace link are owner setup. | apps/api/app/error_tracking.py; apps/api/app/testing.py; .env.example | deferred — owner + phase6 (do NOT tick the live half until staging + Sentry are live) | 2026-06-27 |
| 🔒 | **5.4.2 live half — the web JS error + Web-Vitals event lands under the web DSN in Sentry.** CI half DONE + ticked: `@sentry/react` inits only when `VITE_SENTRY_DSN_WEB` is set (no-op otherwise; `browserTracingIntegration` for perf/Web-Vitals), the hidden `VITE_ENABLE_DEBUG_TOOLS`-gated debug button routes a thrown error through the capture chokepoint, and `e2e/sentry.spec.ts` asserts the capture fired with zero egress (no DSN in the preview build). The live checks — a real JS error event with Web-Vitals context under the **web** project's DSN, and the web↔Grafana `trace_id` link — need the **live web Sentry DSN (`VITE_SENTRY_DSN_WEB`) + the Phase-6 web deploy (owner)**. NOTE: the web Sentry trace does not yet share the backend `traceparent` (5.5 injects it for the API leg, but Sentry-web starts its own browser trace) — unifying web-Sentry↔Tempo by the same `trace_id` is a Phase-6 wiring follow-up. | apps/web/src/lib/error-tracking.ts; apps/web/.env.example; apps/web/src/vite-env.d.ts | deferred — owner + phase6 (CI half ticked; live web DSN + dashboard later) | 2026-06-27 |
| 🔒 | **5.4.3 — Sentry alert rule (new/regressed issue or error-volume spike → team channel). ENTIRELY LIVE + owner.** No CI-verifiable portion: it is a Sentry-dashboard alert rule wired to a real notification channel (Slack/Discord/email). **Left OPEN, NOT ticked.** The trigger it fires on (the debug error) is wired by 5.4.1. Needs the **live Sentry project + a configured channel (owner)**; same family as the Grafana alerts (5.7). | planning/tasks/phase-5-observability.md (5.4.3) | deferred — owner (entirely live) | 2026-06-27 |
| 🔒 | **5.6.1 / 5.6.2 / 5.6.3 live half — the dashboard "renders non-empty after a load script."** CI half DONE + ticked: the Service-health / Cost-guard / Product dashboards are committed AS CODE (`infra/grafana/dashboards/{service-health,cost-guard,product}.json`, portable via a `${datasource}` template var) referencing the REAL emitted metric names, and `apps/api/tests/obs/test_dashboards.py` proves each is valid Grafana JSON AND that every PromQL metric maps to an emitted instrument (AST cross-ref of `app/llm_observability.py`+`app/product_metrics.py` + the FastAPI RED histogram) — correct-by-construction + a metric-rename drift guard. The live check (after a load script the panels render non-empty rate/error/latency, `llm_budget_remaining` falling, non-zero blocks, non-zero reviews/cards/signups/active-users) needs the **Phase-6 staging Cloud Run deploy + live Grafana Cloud / Mimir creds (owner)**. | infra/grafana/dashboards/*.json; apps/api/tests/obs/test_dashboards.py; infra/grafana/README.md | deferred — owner + phase6 (CI half ticked; live render later) | 2026-06-27 |
| 🔒 | **5.6.4 — Infra dashboard. LEFT OPEN (Phase-6).** Depends ENTIRELY on the Phase-6 Cloud Run deploy: no Cloud Run instance/cold-start metrics exist yet, DB-latency needs a histogram wired at deploy, and the Loki error-feed needs logs shipped from a deployed service. A **skeleton** is committed (`infra/grafana/dashboards/infra.json`, tagged `phase6-skeleton` with `${datasource}`/`${loki_datasource}` placeholders) and is intentionally **excluded** from the metric cross-ref (its expressions are placeholders — Cloud Run / GCP-managed-Prometheus names finalize in Phase 6). Not tickable as code. | infra/grafana/dashboards/infra.json; planning/tasks/phase-5-observability.md (5.6.4) | deferred — phase6 (skeleton committed; not ticked) | 2026-06-27 |
| ◐ | **CEFR-level-distribution panel pending a metric (5.6.3).** No per-band CEFR metric is emitted, so the product dashboard's CEFR panel is a documented placeholder **text panel** (not a broken query). To light it up (Phase-6 follow-up): add a `proficiency_cefr_band` observable gauge (labelled by `band`, derived from `lengua_core.proficiency` per active language) to `app/product_metrics.py`, then swap the text panel for a bar gauge / pie over `sum by (band) (proficiency_cefr_band)`. | apps/api/app/product_metrics.py; infra/grafana/dashboards/product.json | follow-up (Phase 6; non-blocking) | 2026-06-27 |
| ◐ | **Histogram metric-name suffix is the one env-specific dashboard knob (5.6.1).** The service-health panels query `http_server_duration_milliseconds{,_bucket,_count}` (route label `http_target`) — the OTLP→Prometheus default (`UnderscoreEscapingWithSuffixes`, ms→milliseconds, old-semconv path label). If the deployed env disables unit suffixes, drop `_milliseconds`; if it upgrades to stable HTTP semconv the metric becomes `http_server_request_duration_seconds` (label `http_route`). The drift test accepts the suffixed + unsuffixed forms; confirm the exact name in Grafana Explore at deploy. | infra/grafana/dashboards/service-health.json; infra/grafana/README.md | confirm at phase6 deploy (test tolerant) | 2026-06-27 |
| 🔒 | **5.9.1 live half — PostHog requests actually reach the EU project.** CI half DONE + ticked: the real adapter (`apps/web/src/lib/posthog.ts`, registered in `main.tsx`) lazily `import('posthog-js')`s (code-split, ~209 kB chunk, NOT the main bundle) and inits at the EU host `https://eu.i.posthog.com` with autocapture/pageview/pageleave/session-recording OFF + `person_profiles:'identified_only'`; capture is gated on consent (the 4.10.3 seam) AND `VITE_POSTHOG_KEY`, so dev/CI/E2E load + send NOTHING (`e2e/consent.spec.ts` proves zero analytics calls before/after Accept with no key), and the keyed path (init at the eu host + opt-out/opt-in) is proven by `lib/posthog.test.ts` (mocked posthog-js). The live check — with a real `VITE_POSTHOG_KEY` in a deployed build, capture requests reach the EU host and events show up in the PostHog project — needs the **owner's PostHog project key (`VITE_POSTHOG_KEY`) + the Phase-6 web deploy**. | apps/web/src/lib/posthog.ts; apps/web/src/lib/analytics.ts; apps/web/src/components/analytics-consent-toggle.tsx; apps/web/.env.example | deferred — owner + phase6 (CI halves ticked; live ingestion later) | 2026-06-27 |
| 🔒 | **5.9.2 live half — the four funnel events appear in PostHog.** CI half DONE + ticked: `lib/analytics-events.ts` fires `signup`/`language_added`/`generate`/`review` through the consent-gated `captureAnalytics` at the four points, with NO PII (`{method}`/`{code}`/`{word_count}`/`{rating}` only); `lib/analytics-events.test.ts` + each integration-point test prove they fire (on success only) with no PII. The live check — completing the funnel as a consented test user and seeing all four events with PII-free payloads in PostHog — needs a real key + deploy (owner), same as 5.9.1. | apps/web/src/lib/analytics-events.ts; apps/web/src/pages/Signup.tsx; apps/web/src/lib/{generate,review,languages}.ts | deferred — owner + phase6 (CI half ticked; live appearance later) | 2026-06-27 |
| 🔒 | **5.9.3 — PostHog funnel / D1-D7 retention / reviews-day / feature-usage insights. ENTIRELY LIVE + owner.** No CI-verifiable portion: building the activation-funnel insight, the D1/D7 retention cohort grid, reviews-per-day, and the Discover / tap-a-word feature-usage insights is dashboard work in the PostHog UI against ingested events. **Left OPEN, NOT ticked.** The activation events it builds on are wired by 5.9.2 (the `review` event also powers reviews/day); the Discover / tap-a-word feature-usage events would be added alongside this when it is built live. Needs the **live PostHog project + ingested events (owner)**; depends 5.9.2. | planning/tasks/phase-5-observability.md (5.9.3) | deferred — owner (entirely live) | 2026-06-27 |
| 🔒 | **5.7.1 — Grafana contact point + notification policy.** As-code DONE + CI-lint-verified (NOT ticked — the verify is live delivery): committed `infra/grafana/alerts/contact-points.yaml` (the `lengua-oncall` contact point — Slack primary; email + Discord alternatives commented; every secret an env-expanded `${...}` placeholder so the repo is secret-free) + `infra/grafana/alerts/notification-policies.yaml` (root policy → `lengua-oncall`, with critical/warning routes adjusting cadence). `apps/api/tests/obs/test_alerts.py` proves both parse + are structurally valid. The live verify — Grafana "Test" on the contact point delivers to the real Slack/Discord/email channel — needs **live Grafana Cloud + a real webhook/address (owner)**, same family as 5.4.3. | infra/grafana/alerts/contact-points.yaml; infra/grafana/alerts/notification-policies.yaml; apps/api/tests/obs/test_alerts.py | deferred — owner (live delivery) | 2026-06-27 |
| 🔒 | **5.7.2 — API 5xx error-rate spike alert.** As-code DONE + CI-lint-verified (NOT ticked — the verify is a live firing on staging traffic): alert rule `lengua-api-5xx-spike` in `infra/grafana/alerts/alert-rules.yaml` (5xx ratio > 5% for 5m) over the REAL `http_server_duration_milliseconds_count{http_status_code=~"5.."}` histogram — same metric as the service-health dashboard; the lint test cross-references its PromQL against the emitted instruments (AST of `app/llm_observability.py`+`app/product_metrics.py` + the FastAPI RED histogram). Live verify = drive forced 5xx on staging → the alert fires to the channel; needs the **Phase-6 staging deploy + Grafana creds (owner)**; depends 5.6.1. | infra/grafana/alerts/alert-rules.yaml; apps/api/tests/obs/test_alerts.py | deferred — owner + phase6 (live firing) | 2026-06-27 |
| 🔒 | **5.7.3 — sustained p95 latency alert.** As-code DONE + CI-lint-verified (NOT ticked — live firing): alert rule `lengua-api-p95-latency` (overall p95 > 1.5s for 10m, `histogram_quantile` over the `_bucket` series). Live verify = inject latency on staging → fires + clears; needs the **Phase-6 staging deploy + Grafana creds (owner)**; depends 5.6.1. | infra/grafana/alerts/alert-rules.yaml | deferred — owner + phase6 (live firing) | 2026-06-27 |
| 🔒 | **5.7.4 — LLM global budget ≥ 80% alert (early warning before the kill-switch).** As-code DONE + CI-lint-verified (NOT ticked — live firing): alert rule `lengua-llm-budget-80pct` over the REAL provider-agnostic `llm_budget_remaining` gauge (NOT the stale `gemini_budget_remaining` — naming reconciliation above) — fires when remaining < 20% of `GLOBAL_DAILY_BUDGET`. The lint test asserts the committed threshold (200) == 20% of the configured `GLOBAL_DAILY_BUDGET` default (1000), so the alert can't silently drift from the gate ceiling. Live verify = push synthetic usage past 80% on staging → fires; needs the **Phase-6 staging deploy + Grafana creds (owner)**; depends 5.6.2. | infra/grafana/alerts/alert-rules.yaml; apps/api/tests/obs/test_alerts.py | deferred — owner + phase6 (live firing) | 2026-06-27 |
| 🔒 | **5.7.5 — prod `/health` uptime alert.** As-code DONE + CI-lint-verified (NOT ticked — live firing): alert rule `lengua-health-uptime-down` over the external `probe_success` series (Grafana Synthetic Monitoring / Blackbox — produced by the 5.8.1 uptime monitor, NOT an app metric, so the lint test allow-lists `probe_success` as the one external metric), `noDataState=Alerting` so a silent probe reads as down. The Grafana mirror of the external prober's own alert (defense in depth). Live verify = point at a failing endpoint → reaches the channel within the interval; needs the **Phase-6 prod deploy + the external monitor + Grafana creds (owner)**; depends 5.8.1. | infra/grafana/alerts/alert-rules.yaml | deferred — owner + phase6 (live firing) | 2026-06-27 |
| 🔒 | **5.8.1 — external uptime monitor (free prober → prod `/health`).** As-code DONE + CI-lint-verified (NOT ticked — the verify is a live external monitor against a real prod URL): committed `infra/uptime/uptime-monitor.yaml` — a vendor-neutral descriptor (`GET ${PROD_BASE_URL}/health`, expect 200 `{"status":"ok"}`, every 3 min, alert-on-failure to the on-call contact) plus ready encodings for BetterStack / UptimeRobot / Grafana Synthetic Monitoring — + `infra/uptime/README.md` + the `docs/runbook.md` "Health checks" entry. `apps/api/tests/obs/test_alerts.py` proves it parses + targets `/health` on a few-minute interval with a contact. Live verify = the monitor shows green at interval and flips DOWN + notifies on failure; needs the **real prod URL (Phase-6 Cloud Run deploy) + a free-prober account (owner)**; depends Phase-6 prod deploy. | infra/uptime/uptime-monitor.yaml; infra/uptime/README.md; docs/runbook.md | deferred — owner + phase6 (external monitor) | 2026-06-27 |

---

## 12. Phase 6 (Infra & CI/CD) — buildable-as-code done; live provisioning owner-deferred

Phase 6 is **in progress**. Each task's **buildable-as-code half** (Dockerfile, endpoints, CI/CD
workflow YAML, IAM/secret descriptors, runbook) lands + stays green under the per-PR gate; the
**live half** (creating GCP projects / Cloud Run services / Artifact Registry / IAM, hosted
Supabase + Vercel projects, Secret Manager / Vercel / Actions secrets, custom SMTP + domains, and
the actual deploys/migrations against those) needs **owner-provisioned cloud resources** and is
recorded here as as-code-done/live-later, NOT ticked. Glyphs: 🔒 owner-blocked · ◐ as-code-done/live-later.

| | Item | Where | Status / decision | Noticed |
|---|---|---|---|---|
| ◐ | **6.1.2 live half — wire the Cloud Run probes.** As-code DONE + CI-verified: the unauthenticated `GET /ready` readiness probe (plain RLS-free-engine `SELECT 1` → `200 {"status":"ready"}` / `503 {"status":"not_ready"}`, never 500) + `GET /health` liveness are both live and tested (`tests/test_health_ready.py`, 100% on the new code). The remaining half — pointing Cloud Run's **startup + liveness** probes at `/health` and its **readiness** probe at `/ready` in the service config — is part of the CD service definition (group 6.6) and needs the **live Cloud Run service (owner/G6)**. The runbook "Health checks → The `/ready` endpoint" documents the intended wiring. | apps/api/app/main.py; apps/api/app/db/session.py; apps/api/tests/test_health_ready.py; docs/runbook.md | deferred — owner + phase6 (endpoints ticked; probe config lands in CD §6.6) | 2026-06-27 |
| ◐ | **6.1.1 local docker verify substituted by CI.** The literal `docker build … && docker run -p 8080:8080 -e PORT=8080 … && curl localhost:8080/health` could not be run in the build environment (Docker daemon not running). Instead the CI `build` job builds the image (existing) **and** a new run-path **smoke step** runs the container with `PORT=8080` and asserts `/health` → 200, so the build+run verify is enforced in CI on every PR. The base image is now **pinned by sha256 digest** (`python:3.12-slim@sha256:6c4dd321…`, the multi-arch OCI index) in both stages. | apps/api/Dockerfile; .github/workflows/ci.yml (build job) | as-code done; build+run path CI-verified (no local Docker daemon) | 2026-06-27 |
| ◐ | **Base-image digest pin needs periodic refresh.** The `python:3.12-slim` digest pin freezes the base OS until bumped, so base-OS security fixes are not picked up automatically. Refresh it (re-resolve the tag's current index digest in both `FROM` lines, kept in lockstep) during deploy hardening, or let Dependabot's `docker` ecosystem bump it — Dependabot itself is owner-deferred (§2, task 0.6.4). | apps/api/Dockerfile | maintenance (refresh at deploy hardening / via Dependabot once enabled) | 2026-06-27 |
| ◐ | **6.10.1 live origin values — set `CORS_ALLOW_ORIGINS` to the real staging/prod web origins at deploy.** As-code DONE + ticked: CORS is env-driven (`CORS_ALLOW_ORIGINS`, NoDecode + field_validator; `CORSMiddleware` with `allow_credentials=True`), defaults to non-wildcard local/Capacitor origins, and `tests/test_cors.py` asserts the default + a prod-shaped allowlist contain no `*`, the actually-installed middleware pairs credentials with a wildcard-free origin list (guards the insecure+invalid `allow_origins=["*"]`+credentials regression), and an un-listed origin is rejected — so the no-wildcard guarantee is enforced in CI in every environment. The remaining half — setting the var to the EXACT deployed staging/prod web origins (the Vercel URLs / custom domains from 6.3.1/6.3.2) in the Cloud Run env / Secret Manager (6.4.1) — needs those **owner-provisioned web origins + the deploy (Phase 6)**. `.env.example` documents the rule + a prod example. | apps/api/app/settings.py; apps/api/app/main.py; apps/api/tests/test_cors.py; .env.example | deferred — owner + phase6 (mechanism + no-wildcard guard ticked; real origin values set at deploy) | 2026-06-27 |
| ◐ | **6.3.4 web-bundle secret-leak audit — DONE as-code, both halves CI-enforced (no live/owner remainder).** The CI `build` job greps the built `dist/` for forbidden server-only env names + provider key value shapes (`gsk_…`/`AIza…`/`service_role`) and fails on any hit (validated zero-false-positive on the clean bundle, catches synthetic leaks, ignores the legitimate anon key); `apps/web/src/lib/bundle-safety.test.ts` (under `pnpm test`) scans the web source for any server-only secret name and locks the `VITE_*` surface to the client-safe allow-list. Listed here only for the Phase-6 record — nothing is owner-deferred. | .github/workflows/ci.yml (build job); apps/web/src/lib/bundle-safety.test.ts | done as-code (CI-enforced); no live half | 2026-06-27 |
| ◐ | **6.8.3 runbook sections — written + TICKED; live staging walkthrough deferred.** As-code DONE: `docs/runbook.md` now has concrete-command sections for **Deploy / Rollback / Run a migration / Rotate a secret / Respond to a budget-exhausted alert / Restore from backup / Store-release checklist** (grounded in the real `lengua-prod` / `lengua-api-{staging,prod}` / `$SUPABASE_{STAGING,PROD}_DATABASE_URL` identifiers, `<placeholder>` for owner values). The verify's first clause (each named section present with concrete, self-contained commands) is met now → box ticked. The second clause — **a reviewer actually follows the rollback section end-to-end on staging** — needs the live Cloud Run service (owner + the §F5 rollback drill in `go-live-activation.md`). | docs/runbook.md | as-code done + ticked; live staging rollback walkthrough deferred (owner; go-live §F5) | 2026-06-27 |
| 🔒 | **6.4.4 secret-rotation procedure — documented (box left UNticked; verify is entirely live).** As-code DONE: `docs/runbook.md` → "Rotate a secret" gives exact `gcloud secrets versions add` → `gcloud run services update …:latest` → verify → revoke steps for the **Groq key**, the **Supabase JWT secret** (lockstep + the zero-downtime JWKS signing-key alternative), and the **deploy credential** (`GCP_SA_JSON` rotation + Workload-Identity-Federation keyless preference). The "script" is the concrete copy-pasteable command sequence in the runbook (the project has no bash-script convention; no separate untested `.sh` added). 6.4.4's verify is **100% live** (rotate the staging Groq key in Secret Manager + redeploy; the service serves on the new key, the old key stops working) → box NOT ticked per the live-verify rule. | docs/runbook.md ("Rotate a secret") | as-code done (procedure); live rotation owner-deferred (do NOT tick) | 2026-06-27 |
| ◐ | **6.2.4 RLS/Supabase-SQL location — RECONCILED as-code (box left UNticked; verify needs a live staging DB).** Added `infra/supabase/README.md` documenting that the canonical Supabase-CLI-applied SQL (RLS policies + `handle_new_user` trigger + kill-switch grants) lives at **repo-root `supabase/migrations/`** (the CLI-native location the local stack + CI apply) — NOT relocated to `infra/supabase/` (that would break `supabase db …`; the plan's "move RLS into infra/supabase/" is intentionally not done). Documents the Alembic-is-DDL-source-of-truth relationship (0002–0004 mirror the Supabase layer; lockstep tests), the per-project apply path (`alembic … upgrade head` / `supabase link` + `supabase db push`), and `infra/README.md` now links it. The live apply (`supabase db push` / Alembic to hosted staging+prod; `pytest tests/test_rls.py` against the staging DB proving cross-tenant isolation) is owner-run. | infra/supabase/README.md; infra/README.md; supabase/migrations/*.sql | as-code reconciled (SQL NOT moved); live apply + RLS-on-staging owner-deferred (do NOT tick) | 2026-06-27 |
| ◐ | **6.2.5 per-env seeding — documented as-code (box left UNticked; verify is a live staging run).** The idempotent seed scripts (`apps/api/scripts/seed_e2e.py` reviewer/demo + due deck; `seed_dev_user.py` fixed dev profile) already exist and select the env from `DATABASE_URL` (+ `SUPABASE_URL`/`SUPABASE_SERVICE_ROLE_KEY` for the Auth-Admin path). `infra/supabase/README.md` → "Seeding per environment" shows the exact per-env invocation + the idempotency guarantee. The live verify (run against hosted staging → reviewer created first run, **no duplicate** second run, reviewer logs in) is owner-run. | infra/supabase/README.md; apps/api/scripts/{seed_e2e,seed_dev_user}.py | as-code documented; live staging seed run owner-deferred (do NOT tick) | 2026-06-27 |
| 🔒 | **6.8.4 backup/restore drill — procedure documented; live drill owner-run.** `docs/runbook.md` → "Restore from backup" documents both Supabase PITR (paid) and the tier-agnostic `pg_dump` → scratch-DB drill (`pg_dump -Fc` → `pg_restore --clean --if-exists` into a scratch DB → `select count(*) from cards`). The verify — actually running the drill against a real prod dump + scratch target and matching row counts — needs the live prod connection string + a scratch DB (owner; go-live track). | docs/runbook.md ("Restore from backup") | as-code documented; live PITR/pg_dump drill owner-deferred (do NOT tick) | 2026-06-27 |
| ◐ | **6.5.1–6.5.8 CI per-PR gate — CONFIRMED + TICKED as-code (CI-enforced every PR); branch-protection ENFORCEMENT owner-deferred.** The blocking gate built in Phase 0 already satisfies every 6.5.x verify and runs on every PR: `setup` (warm caches + `on: pull_request`, 6.5.1); `lint-and-types` (ruff/format/mypy + eslint/prettier/tsc, 6.5.2) which also runs the api-types regeneration drift `git diff --exit-code` (6.5.5); `backend-tests` (Supabase-CLI Postgres + `pytest --cov-branch --cov-fail-under=80` + RLS isolation + `test_openapi_stable.py` asserting `openapi.json == app.openapi()` — the other link of the 6.5.5 contract-drift chain, 6.5.3); `frontend-tests` + `build` (vitest 80% v8 thresholds + the web `dist/` bundle, 6.5.4); `e2e` (ephemeral Supabase + the built API image, `LLM_PROVIDER=fake`, Playwright retries=1, asserting the four critical journeys signup / generate→save / review-four-ratings / 429-path + zero real LLM calls via `/__test__/llm-calls`, 6.5.6); `security` (pip-audit `--strict` + pnpm audit `--audit-level high` + gitleaks, the checkout pinned at `fetch-depth: 0` so gitleaks has full history, 6.5.7); `coverage-comment` + advisory `a11y-perf` (Lighthouse + axe, 6.5.8). Reconciled **purely additively** — no gate weakening, no redundant jobs added (verified the workflow already met each verify). The ONE genuinely owner-deferred half: the green gate is not yet **enforced as a merge requirement** because branch protection on `main` is off (0.6.3, deferred to launch — §2) so solo self-merge keeps working; flip it on at launch to make the gate blocking. | .github/workflows/ci.yml; apps/web/e2e/*.spec.ts; apps/api/tests/test_openapi_stable.py; planning/tasks/phase-6-infra-cicd.md (6.5.x) | confirmed + ticked as-code (CI-enforced every PR); branch-protection enforcement owner-deferred (0.6.3) | 2026-06-28 |
...
| 🔒 | **Web host — MOVING off Vercel → Cloudflare Pages** (decided 2026-06-28). Vercel free = 1 member, so Ben+Kotlar can't both admin it. Plan written in [`go-live-activation.md §H`](go-live-activation.md) (not yet executed): create Cloudflare account + invite the other as admin + API token/account-id, swap the `deploy-web-*` CD jobs Vercel→`wrangler pages deploy`, add SPA fallback, repoint `*_WEB_ORIGIN` + Supabase redirects, update every doc that says "Vercel". Firebase Hosting kept as GCP-native fallback. | `go-live-activation.md §H` + `.github/workflows/` | owner sets up Cloudflare; Ben does the CD swap | 2026-06-28 |
...
| ☐ | **Primary-nav landmark has a zero-size own bounding box below `sm`** (the `<nav>` sizes only at `sm:`; its mobile list is `fixed`). Any FUTURE Playwright assertion `expect(nav).toBeVisible()` at a mobile viewport will fail — today's staging fixtures only assert it at desktop, and the new 390px spec targets `nav-mobile` instead. If mobile staging specs are added, assert on `getByTestId('nav-mobile')`. | `apps/web/src/components/app-layout.tsx` | low — latent test trap, documented | 2026-07-03 |
| ☐ | **Desktop sidebar (`h-[calc(100vh-52px)]`) has no `overflow-y-auto`** — on very short `sm+` viewports (<~560px tall) the CefrPanel bottom block could clip. Review skeptics rejected it as unreachable on real hardware; add `overflow-y-auto` to the nav if short-viewport reports appear. | `apps/web/src/components/app-layout.tsx` | low | 2026-07-03 |
