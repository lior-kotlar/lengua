...
| 4 — React web app | 45 | 🛠 in progress — **4.1 shell + 4.2 typed client + 4.3 auth + 4.4 languages/CEFR + 4.5 Generate + 4.6 Review + 4.7 Discover DONE** (30 boxes); remaining: settings/account, RTL+diacritics, consent, Streamlit retirement |
...
| ☑ | **Settings read-only data layer added (`lib/settings.ts`).** Just `useSettingsQuery` (`GET /settings`) + the `discover_count` key, for Discover's default count. The full Settings editor (mutation + bounds validation) is group 4.8, which will reuse this query. | apps/web/src/lib/settings.ts | done in 4.7 | 2026-06-27 |
