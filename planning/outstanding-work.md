...
| 4 — React web app | 45 | 🛠 in progress — **4.1 shell + 4.2 typed client + 4.3 auth + 4.4 languages/CEFR + 4.5 Generate + 4.6 Review + 4.7 Discover DONE; 4.8 settings/account PR open (PAUSE — manager review)** (33 boxes); remaining: RTL+diacritics, consent, Streamlit retirement |
...
| 🔧 | **Backend gap: review batch ignores the per-user `daily_new_limit`/`daily_total_limit` (note for a later backend task).** `app/routers/review.py` → `ReviewService.due_split` uses the `lengua_core.config` defaults and does NOT read these `user_settings` keys, so editing them in the web Settings screen persists them but does not yet change the due batch. Out of scope for this web group; a small backend wiring task (read the user's settings into `due_split`). The web screen + persistence are correct now. | apps/api/app/routers/review.py; app/services/review.py | note (backend follow-up) | 2026-06-27 |
...
| 🔧 | **ESCALATED — daily new/total limit settings persist but have ZERO effect (parity regression).** The review confirmed the backend gap below is user-visible: the Settings copy claims a live effect, but `review_due` never reads `daily_new_limit`/`daily_total_limit`, so the legacy Streamlit behaviour is lost. **MUST be fixed before the Phase-4 exit gate** — scheduled as a dedicated backend PR **4.8b** in Workflow B (wire `review_due` → `due_split(new_limit, total_limit)` from the user's settings; the service already accepts the kwargs; + fix the false `app/schemas/settings.py` docstring "the Phase 3 quota gate reads these"; + add a test that a per-user limit bounds the batch; + cross-field `new ≤ total` form validation). | apps/api/app/routers/review.py; app/services/review.py; app/schemas/settings.py; apps/web/src/lib/settings.ts | tracked must-fix (4.8b) | 2026-06-27 |
